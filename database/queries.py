...
# =====================================================
# GUIDED INQUIRY: VALIDASI DAN UTILITAS
# =====================================================

INQUIRY_RESPONSE_FIELDS = {
    "pengamatan",
    "rumusan_masalah",
    "hipotesis",
    "alasan_hipotesis",
...
def now_iso():
    """Timestamp ISO dengan zona waktu untuk tabel guided inquiry."""
    return datetime.now().astimezone().isoformat(timespec="seconds")


def _validate_id_user(id_user):
    try:
        value = int(id_user)
    except (TypeError, ValueError) as exc:
        raise ValueError("id_user tidak valid.") from exc
    if value <= 0:
        raise ValueError("id_user tidak valid.")
    return value


def _validate_mission_code(mission_code):
    mission_code = _normalize_text(mission_code)
    if mission_code not in MISSION_CODES:
        raise ValueError("Kode misi tidak valid.")
    return mission_code


def _validate_stage_code(stage_code):
    stage_code = _normalize_text(stage_code)
    if stage_code not in INQUIRY_STAGES:
        raise ValueError("Tahap guided inquiry tidak valid.")
    return stage_code


def _to_jsonable(value):
    """Mengubah nilai pandas/numpy/datetime menjadi tipe yang aman untuk JSONB."""
...
# =====================================================
# GUIDED INQUIRY: PROGRESS PER MISI
# =====================================================

def initialize_mission_progress(id_user):
    """Memastikan satu siswa memiliki empat baris progress misi.

    Fungsi ini idempotent dan aman dipanggil setiap kali dashboard dibuka.
    """
    id_user = _validate_id_user(id_user)
    existing_df = get_mission_progress_df(id_user)
    existing_codes = set(existing_df["mission_code"].astype(str)) if not existing_df.empty else set()

    missing_codes = [code for code in MISSION_CODES if code not in existing_codes]
    if missing_codes:
        timestamp = now_iso()
        payload = [
            {
                ...
                "current_stage": INQUIRY_STAGES[0],
                "status": "belum_dimulai",
                "started_at": None,
                "completed_at": None,
                "updated_at": timestamp,
            }
            for mission_code in missing_codes
        ]
        client().table("mission_progress").insert(payload).execute()

    return get_mission_progress_df(id_user)


def get_mission_progress_df(id_user):
    id_user = _validate_id_user(id_user)
    response = (
        client()
...
def get_mission_progress(id_user, mission_code):
    id_user = _validate_id_user(id_user)
...
def start_mission(id_user, mission_code):
    """Memulai misi tanpa mereset progress yang sudah ada."""
    id_user = _validate_id_user(id_user)
    mission_code = _validate_mission_code(mission_code)
    initialize_mission_progress(id_user)
    current = get_mission_progress(id_user, mission_code)

    if current and current.get("status") == "belum_dimulai":
        timestamp = now_iso()
        client().table("mission_progress").update({
            "status": "sedang_dikerjakan",
            "current_stage": INQUIRY_STAGES[0],
            "started_at": timestamp,
            "completed_at": None,
            "updated_at": timestamp,
        }).eq("id_user", id_user).eq("mission_code", mission_code).execute()

    return get_mission_progress(id_user, mission_code)


def set_mission_stage(id_user, mission_code, stage_code):
    """Mengatur tahap aktif tanpa menandai misi selesai."""
    id_user = _validate_id_user(id_user)
    mission_code = _validate_mission_code(mission_code)
    stage_code = _validate_stage_code(stage_code)
...
def advance_mission_stage(id_user, mission_code, completed_stage):
    """Memajukan misi satu tahap setelah tahap aktif selesai.

    Fungsi menolak lompatan tahap. Pemanggilan ulang pada tahap yang sudah lewat
    bersifat idempotent dan tidak menurunkan progress.
    """
    id_user = _validate_id_user(id_user)
    mission_code = _validate_mission_code(mission_code)
    completed_stage = _validate_stage_code(completed_stage)
    current = start_mission(id_user, mission_code)

    current_stage = current.get("current_stage", INQUIRY_STAGES[0])
    current_index = INQUIRY_STAGES.index(current_stage)
    completed_index = INQUIRY_STAGES.index(completed_stage)
    ...
    if completed_index < current_index:
        return current
    if completed_index > current_index:
        raise ValueError("Tahap belum dapat diselesaikan karena tahap sebelumnya belum selesai.")

    if completed_stage == INQUIRY_STAGES[-1]:
        return complete_mission(id_user, mission_code)

    return set_mission_stage(id_user, mission_code, get_next_stage(completed_stage))


def complete_mission(id_user, mission_code):
    id_user = _validate_id_user(id_user)
    mission_code = _validate_mission_code(mission_code)
    start_mission(id_user, mission_code)
    ...
    timestamp = now_iso()
    client().table("mission_progress").update({
        "current_stage": INQUIRY_STAGES[-1],
        "status": "selesai",
...
def get_completed_mission_count(id_user):
    progress_df = initialize_mission_progress(id_user)
    if progress_df.empty:
        return 0
    return int((progress_df["status"].astype(str) == "selesai").sum())


# =====================================================
# GUIDED INQUIRY: JAWABAN SISWA
# =====================================================

def get_inquiry_response(id_user, mission_code):
    id_user = _validate_id_user(id_user)
    mission_code = _validate_mission_code(mission_code)
    response = (
        client()
...
def get_inquiry_responses_df(id_user=None, mission_code=None):
    query = client().table("inquiry_responses").select("*")
    if id_user is not None:
        query = query.eq("id_user", _validate_id_user(id_user))
    if mission_code is not None:
        query = query.eq("mission_code", _validate_mission_code(mission_code))

    response = query.order("updated_at", desc=True).execute()
    return _df(_data(response))


def save_inquiry_response(id_user, mission_code, **fields):
    """Menyimpan sebagian atau seluruh jawaban guided inquiry.

    Hanya kolom pada INQUIRY_RESPONSE_FIELDS yang dapat diubah. Data lama pada
    kolom lain tetap dipertahankan.
    """
    id_user = _validate_id_user(id_user)
    mission_code = _validate_mission_code(mission_code)

    unknown_fields = set(fields) - INQUIRY_RESPONSE_FIELDS
    if unknown_fields:
        raise ValueError(f"Kolom jawaban tidak valid: {', '.join(sorted(unknown_fields))}")
    if not fields:
        raise ValueError("Tidak ada jawaban yang disimpan.")

    payload = {}
    for key, value in fields.items():
        if key == "status_hipotesis":
            normalized = _normalize_text(value).lower() if value is not None else ""
            if normalized and normalized not in HYPOTHESIS_STATUS:
                raise ValueError("Status hipotesis tidak valid.")
    ...
    existing = get_inquiry_response(id_user, mission_code)

    if existing:
        client().table("inquiry_responses").update(payload).eq(
            "id_response", existing["id_response"]
        ).execute()
    else:
        payload.update({
            "id_user": id_user,
        ...
        client().table("inquiry_responses").insert(payload).execute()

    return get_inquiry_response(id_user, mission_code)


# =====================================================
# GUIDED INQUIRY: HASIL PERCOBAAN SIMULASI
# =====================================================

def get_experiment_runs_df(id_user, mission_code):
    id_user = _validate_id_user(id_user)
    mission_code = _validate_mission_code(mission_code)
    response = (
        client()
...
def get_next_run_number(id_user, mission_code):
    runs_df = get_experiment_runs_df(id_user, mission_code)
    if runs_df.empty:
        return 1
    return int(runs_df["run_number"].max()) + 1
...
def save_experiment_run(
    ...
    """Menyimpan satu percobaan simulasi.

    Bila run_number sudah ada, data percobaan tersebut diperbarui. Ini membuat
    tombol "Jalankan ulang" aman digunakan tanpa membuat duplikasi.
    """
    id_user = _validate_id_user(id_user)
    mission_code = _validate_mission_code(mission_code)
    simulation_code = _normalize_text(simulation_code)
    if not simulation_code:
        simulation_code = MISSIONS[mission_code]["simulation_code"]

    if run_number is None:
        run_number = get_next_run_number(id_user, mission_code)
    try:
        run_number = int(run_number)
    except (TypeError, ValueError) as exc:
        raise ValueError("Nomor percobaan tidak valid.") from exc
    if run_number <= 0:
        raise ValueError("Nomor percobaan harus lebih dari 0.")
    ...
    payload = {
        ...
        "input_data": _to_jsonable(input_data or {}),
        "output_data": _to_jsonable(output_data or {}),
    }
    ...
    if existing:
        client().table("experiment_runs").update(payload).eq(
            "id_run", existing["id_run"]
        ).execute()
    else:
        payload["created_at"] = now_iso()
        client().table("experiment_runs").insert(payload).execute()

    return get_experiment_runs_df(id_user, mission_code)


def count_experiment_runs(id_user, mission_code):
    return int(len(get_experiment_runs_df(id_user, mission_code)))


def has_minimum_experiment_runs(id_user, mission_code):
    mission_code = _validate_mission_code(mission_code)
